...
class Strauss(nn.Module):
    ...
    def _euclid_dist(self, X):
        Xs = torch.sum(torch.square(X), dim=-1, keepdim=True)
        dist = -2 * X @ X.T
        dist += Xs + Xs.T

        return torch.sqrt(torch.maximum(dist, torch.tensor(1e-40, dtype=dist.dtype)))
...
class BSGP(nn.Module):
    def __init__(self, X, Y, kernel, likelihood, prior_type, inputs, outputs,
        ...
        if inducing_points_init is not None:
            ...
            inducing_points_init = torch.tensor(kmeans2(X, self.M, minit='points')[0], dtype=torch.get_default_dtype())

        if prior_type == "strauss":
            self.pZ = Strauss(R=0.5)

        self.Z = nn.Parameter(inducing_points_init, requires_grad=True)
        self.U = nn.Parameter(torch.zeros((self.M, self.outputs), dtype=torch.get_default_dtype()), requires_grad=True)
        self.Lm = None
    ...
    def log_prior_Z(self):
        ...
        if self.prior_type == "determinantal":
            self.Lm = torch.linalg.cholesky(
                self.kern.K(self.Z) + torch.eye(self.M, dtype=torch.float64, device=self.Z.device) * 1e-7
            )
    ...
    def conditional(self, X):
        mean, var, self.Lm = conditional(
            X, self.Z, self.kern, self.U, whiten=True, full_cov=self.full_cov, return_Lm=True
        )
        return mean, var
